...
"use client"
import { RefreshCw, AlertTriangle, Crown, Zap, Wifi, WifiOff, Lock } from "lucide-react";
...
const RequestFooter = () => {
  ...
  useEffect(() => {
    if (requestStatus && !requestStatus.canMakeRequest && !loading && !error) {
  ...
  return (
    <>
      ...
      {/* Full-screen Limit Reached Modal */}
      {showLimitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="relative w-full max-w-md mx-4">
            {/* Modal content */}
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-slate-600/50 p-8 shadow-2xl">
              {/* Icon */}
              <div className="flex justify-center mb-6">
                <div className="flex items-center justify-center w-16 h-16 bg-gradient-to-br from-orange-500 to-red-600 rounded-full">
              ...
              <p className="text-slate-400 text-center mb-6">
                You`ve used all your free requests for today. Upgrade to continue using our service without interruption.
              </p>

              {/* Stats */}
              <div className="bg-slate-800/50 rounded-lg p-4 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-slate-300">Requests Used</span>
                  <span className="text-white font-semibold">{requestStatus?.requestCount || 0}/5</span>
                </div>
                <div className="w-full bg-slate-700 rounded-full h-2 mt-2">
                  <div className="bg-gradient-to-r from-orange-500 to-red-600 h-2 rounded-full w-full"></div>
                </div>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-slate-700">
                <p className="text-xs text-slate-500 text-center">
                  Premium includes unlimited requests, priority support, and advanced features
                </p>
              </div>
            </div>
          </div>
